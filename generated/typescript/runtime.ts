...
export function loadFrom(lookup: (key: string) => string | undefined): CliEnvValues {
  return {
    api_url: nonEmpty(lookup("APME_API_URL")) ?? "http://127.0.0.1:8080",
    output: nonEmpty(lookup("APME_OUTPUT")) ?? "json",
    timeout_seconds: parseIntValue(lookup("APME_TIMEOUT_SECONDS"), 20),
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const api_url = pick(["APME_API_URL"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "http://127.0.0.1:8080");
  if (api_url !== undefined) out["APME_API_URL"] = api_url;
  const output = pick(["APME_OUTPUT"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "json");
  if (output !== undefined) out["APME_OUTPUT"] = output;
  const timeout_seconds = pick(["APME_TIMEOUT_SECONDS"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "20");
  if (timeout_seconds !== undefined) out["APME_TIMEOUT_SECONDS"] = timeout_seconds;
  return out;
}
