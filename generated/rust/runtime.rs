...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        api_url: lookup("APME_API_URL").filter(|value| !value.is_empty()).unwrap_or_else(|| "http://127.0.0.1:8080".to_string()),
        output: lookup("APME_OUTPUT").filter(|value| !value.is_empty()).unwrap_or_else(|| "json".to_string()),
        timeout_seconds: parse_int(lookup("APME_TIMEOUT_SECONDS"), 20),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let api_url = pick(&["APME_API_URL"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("http://127.0.0.1:8080"));
    if let Some(value) = api_url {
        out.insert("APME_API_URL".to_string(), value);
    }
    let output = pick(&["APME_OUTPUT"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("json"));
    if let Some(value) = output {
        out.insert("APME_OUTPUT".to_string(), value);
    }
    let timeout_seconds = pick(&["APME_TIMEOUT_SECONDS"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("20"));
    if let Some(value) = timeout_seconds {
        out.insert("APME_TIMEOUT_SECONDS".to_string(), value);
    }
    Ok(out)
}
