...
final class CliEnvValues {
  const CliEnvValues({required this.apiUrl, required this.output, required this.timeoutSeconds});
...
CliEnvValues loadFrom(String? Function(String key) lookup) {
  return CliEnvValues(
    apiUrl: _nonEmpty(lookup('APME_API_URL')) ?? 'http://127.0.0.1:8080',
    output: _nonEmpty(lookup('APME_OUTPUT')) ?? 'json',
    timeoutSeconds: _parseInt(lookup('APME_TIMEOUT_SECONDS'), 20),
  );
}
...
Map<String, String> loadEnvMap(
  ...
  final out = <String, String>{};
  final apiUrl = pick(['APME_API_URL'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'http://127.0.0.1:8080');
  if (apiUrl != null) out['APME_API_URL'] = apiUrl;
  final output = pick(['APME_OUTPUT'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, 'json');
  if (output != null) out['APME_OUTPUT'] = output;
  final timeoutSeconds = pick(['APME_TIMEOUT_SECONDS'], ['flags', 'env_shell', 'env_file'], shell, dotenv, flags, '20');
  if (timeoutSeconds != null) out['APME_TIMEOUT_SECONDS'] = timeoutSeconds;
  return out;
}
